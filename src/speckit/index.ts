...
class EnterpriseSpecKitManager {
  ...
  async initializeProject(args: z.infer<typeof InitProjectSchema>): Promise<string> {
    const { projectName, aiAssistant, workingDirectory, useCurrentDirectory, enterpriseMode, directoryConfig } = args;
    ...
    // Copy templates to configured directories
    const specTemplate = await this.readTemplate('spec-template.md');
    const planTemplate = await this.readTemplate('plan-template.md');
    const tasksTemplate = await this.readTemplate('tasks-template.md');
    const constitution = await this.readTemplate('constitution.md');
    ...
    const assistantCommands = {
      claude: await this.readTemplate('commands/specify.md'),
      gemini: await this.readTemplate('commands/specify.md'),
      copilot: await this.readTemplate('commands/specify.md')
    };
    
    const commandsDir = this.getDirectoryPath(projectPath, 'commands');
    await this.createDirectory(commandsDir);
    await fs.writeFile(path.join(commandsDir, 'specify.md'), assistantCommands[aiAssistant]);
    await fs.writeFile(path.join(commandsDir, 'plan.md'), await this.readTemplate('commands/plan.md'));
    await fs.writeFile(path.join(commandsDir, 'tasks.md'), await this.readTemplate('commands/tasks.md'));
    ...
    if (this.monitoring) {
      await this.monitoring.log({
        ...
        metadata: { projectPath, aiAssistant, enterpriseMode }
      });
    }
  ...
  async createSpecification(args: z.infer<typeof CreateSpecificationSchema>): Promise<string> {
    ...
    const template = await this.readTemplate('spec-template.md');
  ...
  async createImplementationPlan(args: z.infer<typeof CreatePlanSchema>): Promise<string> {
    ...
    // Read and customize template
    const template = await this.readTemplate('plan-template.md');
  ...
  async createTasks(args: z.infer<typeof CreateTasksSchema>): Promise<string> {
    ...
    // Read and customize template
    const template = await this.readTemplate('tasks-template.md');
  ...
  async validateConstitution(args: z.infer<typeof ValidateConstitutionSchema>): Promise<string> {
    ...
    const constitutionFile = constitutionPath || path.join(basePath, 'memory', 'constitution.md');
...
server.setRequestHandler(ListToolsRequestSchema, async () => {
  return {
    tools: [
      ...
      {
        name: "init_project",
        description: "Initialize a new Spec-Driven Development project with optional enterprise features",
        inputSchema: {
          type: "object",
          properties: {
