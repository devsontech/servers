...
class SolutionReferenceDiscovery {
    ...
    async analyzeSolution(solutionPath) {
        ...
        const projectAnalyses = await Promise.all(projects.map(proj => this.analyzeProject(proj.projectPath, proj.projectName)));
     ...
     */
    parseProjectXml(xmlContent) {
        // Simple regex-based XML parsing for .csproj structure
        const result = { PropertyGroup: [], ItemGroup: [] };
    ...
    extractTargetFramework(project) {
    ...
    extractPackageReferences(project) {
        const references = [];
    ...
    extractProjectReferences(project, projectDir) {
        const references = [];
        const itemGroups = project.ItemGroup || [];
        for (const group of itemGroups) {
    ...
    async findSourceFiles(projectDir, targetFramework) {
        const sourceFiles = [];
    ...
    determineOutputPath(project, projectDir, targetFramework) {
        const propertyGroups = project.PropertyGroup || [];
        for (const group of propertyGroups) {
            if (group.OutputPath) {
    ...
    determineProjectType(project, sourceFiles) {
        const propertyGroups = project.PropertyGroup || [];
        // Check SDK style
        const projectElement = project.$;
        if (projectElement && projectElement.Sdk) {
            if (projectElement.Sdk.includes('Web'))
                return 'ASP.NET Core Web';
            if (projectElement.Sdk.includes('Worker'))
        ...
        if (sourceFiles.some(f => f.includes('Program.cs')))
            return 'Console/Web Application';
        if (sourceFiles.some(f => f.endsWith('.razor') || f.endsWith('.cshtml')))
    ...
    calculateBuildOrder(dependencyGraph) {
    ...
    buildPackageDependencyMap(projects) {
     ...
     */
    async findSymbolReferences(solutionAnalysis, symbolName) {
    ...
    findProjectForFile(solutionAnalysis, filePath) {
     ...
     */
    async getDotnetInfo(solutionPath) {
    ...
    try {
        ...
        switch (name) {
            case 'analyze_solution':
                const solutionPath = args.solutionPath;
                if (!solutionPath) {
            ...
            case 'find_symbol_references':
                const solutionPathForSearch = args.solutionPath;
