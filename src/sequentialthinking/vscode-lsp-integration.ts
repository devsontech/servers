 ...
 * Uses existing language servers from VS Code extensions instead of installing separately
 */
 ...
 */
export const VSCodeLanguageServers: Record<string, VSCodeLSPConfig> = {
 ...
 */
export function findVSCodeLanguageServer(languageId: string): VSCodeLSPConfig | null {
...
export class VSCodeLSPClient {
  ...
  private config: VSCodeLSPConfig;
  private messageId = 1;

  constructor(languageId: string, config: VSCodeLSPConfig) {
  ...
  private isVSCodeBuiltIn(): boolean {
    return this.config.extensionId.startsWith('vscode.');
  }

  private async startBuiltInServer(rootUri: string): Promise<void> {
  ...
  private async startExtensionServer(rootUri: string): Promise<void> {
 ...
 */
export function createVSCodeLSPClient(languageId: string): VSCodeLSPClient | null {
