...
interface MultiRootWorkspaceInfo {
...
interface EnhancedThoughtData {
  ...
  workspaceContext?: MultiRootWorkspaceInfo;
  languageContext?: {
    languageId: string;
    filePath?: string;
...
class AutoDetectingWorkspaceLSPClient {
  ...
  private workspaceInfo: MultiRootWorkspaceInfo;
   ...
   */
  private detectWorkspaceStructure(): MultiRootWorkspaceInfo {
   ...
   */
  private findWorkspaceFile(startDir: string): string | undefined {
   ...
   */
  private parseWorkspaceFile(workspaceFilePath: string): WorkspaceFolder[] {
   ...
   */
  private detectMultiRootPattern(baseDir: string): WorkspaceFolder[] {
    const folders: WorkspaceFolder[] = [];
   ...
   */
  private hasProjectFiles(dirPath: string): boolean {
    const projectFiles = [
    ...
    try {
      ...
      return projectFiles.some(file => 
        items.some(item => 
          file.startsWith('.') ? item.endsWith(file) : item === file
        )
      );
    } catch (error) {
      return false;
  ...
  async initializeLanguageSupport(languageIds: string[]): Promise<void> {
    for (const languageId of languageIds) {
      const languageClients = new Map<string, any>();
   ...
   */
  async searchAcrossWorkspaces(query: string): Promise<any> {
    const results: any[] = [];
  ...
  private async searchInWorkspace(workspace: WorkspaceFolder, query: string): Promise<any[]> {
    const results: any[] = [];
    ...
    const searchDir = (dir: string) => {
      ...
      for (const item of items) {
        ...
        if (stat.isDirectory()) {
          if (['node_modules', '.git', 'bin', 'obj', 'dist'].includes(item)) continue;
          searchDir(fullPath);
...
class EnhancedSequentialThinking {
  private thoughts: EnhancedThoughtData[] = [];
  private thoughtCounter = 0;
  private lspClient: AutoDetectingWorkspaceLSPClient;
  ...
  async startThinking(query: string): Promise<EnhancedThoughtData> {
    const workspaceInfo = this.lspClient.getWorkspaceSummary();
    
    const thought: EnhancedThoughtData = {
