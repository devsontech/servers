...
export class ContextAwareAssistant {
    ...
    async analyzeCurrentWorkspace() {
        ...
        try {
            ...
            vscode.window.showInformationMessage(`Workspace analyzed: ${analysis.summary.totalFiles} files, complexity: ${analysis.summary.complexity}`, 'View Details').then((selection) => {
    ...
    async analyzeCurrentFile() {
        ...
        try {
            ...
            if (relatedFiles.length > 0) {
                vscode.window.showInformationMessage(`Found ${relatedFiles.length} related files`, 'Show Related').then((selection) => {
    ...
    showContextPanel(data) {
        if (this.contextPanel) {
            ...
            this.contextPanel = vscode.window.createWebviewPanel('contextGen', 'Project Context', vscode.ViewColumn.Beside, { enableScripts: true });
      ...
      <body>
        ...
        <div class="section">
          ...
          ${patterns.detectedPatterns.map(pattern => `
            ...
            (pattern.benefits?.join(', ') || 'Positive pattern detected')}</p>
    ...
    async updateContextForFile(fileName) {
    ...
    async getRelatedContexts(currentFunction, fileName) {
        return [
            {
                id: currentFunction,
                type: 'function',
    ...
    generateSuggestions(prioritizedContext, position, document) {
        const suggestions = [];
    ...
    showSuggestions(suggestions) {
        if (suggestions.length > 0) {
            vscode.window.showInformationMessage(suggestions[0], ...suggestions.slice(1, 3).map(s => s.split(':')[0]));
        }
    }
    async findRelatedFiles(semanticContext) {
        const relatedFiles = [];
    ...
    showFileContext(context, fileName) {
        const entity = context.entities[0];
        if (entity) {
            vscode.window.showInformationMessage(`${entity.entity}: ${entity.purpose}`, 'Show Details').then((selection) => {
    ...
    createKnowledgeGraphPanel(knowledgeGraph) {
        const panel = vscode.window.createWebviewPanel('knowledgeGraph', 'Knowledge Graph', vscode.ViewColumn.Beside, { enableScripts: true });
        panel.webview.html = this.getKnowledgeGraphHTML(knowledgeGraph);
    }
    getKnowledgeGraphHTML(graph) {
        return `
      <!DOCTYPE html>
      <html>
      <head>
      ...
      <body>
        <svg></svg>
        <script>
          const nodes = ${JSON.stringify(graph.nodes.map((node) => ({
            id: node.id,
            title: node.title,
            type: node.type,
          ...
          const links = ${JSON.stringify(graph.relationships.map((rel) => ({
            source: rel.source,
            target: rel.target,
            type: rel.type,
